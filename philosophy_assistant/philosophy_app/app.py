...
from db import save_conversation, save_feedback, get_recent_conversations, get_feedback_stats, feedback_already_exists
from db import init_table_if_needed,check_tables_exist
...
def main():
    ...
    #print init tables if needed
    init_table_if_needed()

    print_log("Starting the Philosophy Assistant application")
    ...
    ideology = st.selectbox(
        "Select a ideology:",
        ["stoicism", "zen-buddhism"]
    )
    print_log(f"User selected ideology: {ideology}")
